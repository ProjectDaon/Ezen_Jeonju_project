...
var searchmap = new kakao.maps.Map(container, options); //지도 생성 및 객체 리턴
...
function placesSearchCB(data, status, pagination, bounds) {
    if (status === kakao.maps.services.Status.OK) {
        ...
        if (data.length > 0) {
            ...
            searchmap.setCenter(placePosition);
            searchmap.setLevel(4);
        }
...
function addMarker(position, idx, title) {
    ...
    marker.setMap(searchmap); // 지도 위에 마커를 표출합니다
...
kakao.maps.event.addListener(searchmap, 'idle', function() {
    searchAddrFromCoords(searchmap.getCenter(), displayCenterInfo);
});
